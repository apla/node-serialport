...
function propVal(name, val) {
  ...
  if (name === 'locationId') {
    let busChunk = false;
    let locationId;
    // /devices/pci0000:00/0000:00:1d.7/usb8/8-6/8-6.1/8-6.1.4/8-6.1.4:1.0/ttyUSB8/tty/ttyUSB8
    val.split('/').forEach(function(chunk, idx, chunks) {
      if (chunk.match(/^usb\d+$/)) return busChunk = true;
      if (busChunk && chunk.match(/^\d+-\d+(?:\.\d+)*:\d+\.\d+$/) && !locationId) {
        locationId = chunks[idx - 1];
      }
    });
    return locationId;
  }
